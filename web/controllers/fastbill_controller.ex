defmodule TimeTracking.FastbillController do
  ...
  def create_project(conn, %{"id" => external_id, "name" => name, "cid" => external_client_id}) do
    ...
    api_return = case @fastbill_api.find_project(%{external_id: "toggl:#{external_id}", client_id: client_id}) do
      {:not_found, _} ->
        @fastbill_api.create_project(%{client_id: client_id, external_id: "toggl:#{external_id}", name: name})
      res ->
        res
    end
